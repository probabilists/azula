...
class SanaDenoiser(GaussianDenoiser):
    ...
    def forward(
        ...
        **kwargs,
        ...
        Returns:
            The Gaussian :math:`\mathcal{N}(X \mid \mu_\phi(x_t \mid y), \Sigma_\phi(x_t \mid y)`.
        """
        ...
        mean = c_skip * x_t + c_out * output.to(x_t)
        var = c_var
...
def load_model(
    ...
    **kwargs,
    ...
    if card.dtype == "float16":
        dtype = torch.float16
        variant = "fp16"
    elif card.dtype == "bfloat16":
        dtype = torch.bfloat16
        variant = "bf16"
    else:
        raise ValueError(f"Unknown dtype '{card.dtype}'.")
